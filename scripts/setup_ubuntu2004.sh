...
echo '> Updating the guest operating system, and install necessary packages ...'
sudo apt-get update
sudo apt-get install -y open-vm-tools openssh-server net-tools perl open-iscsi ntp curl vim ifupdown zip unzip gnupg2 software-properties-common apt-transport-https ca-certificates lsb-release python3-pip jq cloud-init
...
echo '> Enable custom scripts ...'
vmware-toolbox-cmd config set deployPkg enable-custom-scripts true

# Delete netplan config
echo '> Delete netplan config'
